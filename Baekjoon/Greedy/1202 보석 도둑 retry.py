...
for k in range(b):
    ...
    value_sofar = [0]*(weight[k]+1)
    for i in range(1, n+1):
        # 물건의 무게와 가치 (w, v)
        w, v = value[i][0], value[i][1]
        if weight[k]+1 < w: # 현재 허용된 무게보다 넣으려는 물건의 무게가 더 무겁다면
            value_sofar[i] = value_sofar[i-1] # 넣지 않는다. (직전 무게와 동일)
        else: # 현재 허용된 무게 안에 드는 무게라면
            # 1. 현재 물건을 넣는다.
            # 2. 현재 물건을 넣지 말고 이전 배낭 그대로 가지고 간다.
            # 위 둘 중 가치가 큰걸 선택한다.
            if v > value_sofar[i-1]:
                value_sofar[i] = v
                idx = i
            else:
                value_sofar[i] = value_sofar[i-1]
                idx = i-1
            break
    result += value[idx][1]
    value[idx] = [0, 0, 0]
